...
# ── Global JSON error handlers (prevents plain-text leaking to frontend) ──────
...
def unhandled_exception(e):
    return jsonify({'error': f'Unexpected error: {str(e)}', 'details': traceback.format_exc()}), 500
...
def upload_token():
    """
    Returns a Vercel Blob client upload token so the browser can PUT
    the PDF directly to Blob storage (bypassing the 4.5 MB edge limit).
    Requires BLOB_READ_WRITE_TOKEN env var set in Vercel dashboard.
    """
    try:
        blob_token = os.environ.get('BLOB_READ_WRITE_TOKEN', '')
        if not blob_token:
            return jsonify({'error': 'Blob storage not configured. Set BLOB_READ_WRITE_TOKEN in Vercel environment variables.'}), 500

        body = request.get_json(force=True, silent=True) or {}
        filename = secure_filename(body.get('filename', 'plan.pdf'))
        content_type = body.get('content_type', 'application/pdf')

        # Call Vercel Blob API to get a client upload URL
        ...
        if not resp.ok:
            return jsonify({'error': f'Blob API error: {resp.status_code} {resp.text[:200]}'}), 502

        data = resp.json()
        return jsonify({
            'upload_url': data.get('uploadUrl') or data.get('url'),
            'blob_url':   data.get('url'),
            'token':      data.get('clientToken', blob_token)
        })
...
def analyze_plan():
    ...
    Accepts JSON body with file_url (Vercel Blob URL) + metadata.
    Downloads the PDF from Blob, extracts text, runs all 5 analysis modules,
    and returns a structured O3-schema stress-test report.
    """
    ...
    try:
        ...
        if not body:
            return jsonify({'error': 'Request body must be JSON with file_url field.'}), 400
        ...
        county_name = body.get('county', 'Unknown County')
        state       = body.get('state', 'Unknown State')
        plan_year   = body.get('plan_year', 'Unknown')
        plan_type   = body.get('plan_type', 'Single Jurisdiction')
        plan_title  = body.get('plan_title', 'Hazard Mitigation Plan')

        # ── Download PDF from Vercel Blob ──────────────────────────────────────────
        try:
            dl = http_requests.get(file_url, timeout=30, stream=True)
            dl.raise_for_status()
        except Exception as e:
            return jsonify({'error': f'Failed to download PDF from storage: {str(e)}'}), 502

        # Write to /tmp for pdfplumber/PyPDF2
        tmp_fd, filepath = tempfile.mkstemp(suffix='.pdf', dir='/tmp')
        try:
            with os.fdopen(tmp_fd, 'wb') as f:
                for chunk in dl.iter_content(chunk_size=65536):
                    f.write(chunk)
        except Exception as e:
            return jsonify({'error': f'Failed to save PDF locally: {str(e)}'}), 500

        # Get page count before extraction (for metadata)
        total_pages = pdf_processor.get_page_count(filepath)
        pages_analyzed = min(total_pages, 120) if total_pages > 0 else 'unknown'
        ...
        results = {
            ...
            'metadata': {
                ...
                'note': (
                    f'Analysis based on first {pages_analyzed} of {total_pages} pages '
                    '(Vercel Hobby 60s limit).'
                    if total_pages > 120 else
                    f'Full document analyzed ({total_pages} pages).'
                )
            },
