...
# ── Vercel path fix: add the api/ directory to sys.path ──────────────────────
...
# Import analyzers
from analyzer.cfr_compliance import CFRComplianceAnalyzer
from analyzer.bric_analysis import BRICAnalyzer
from analyzer.equity_justice import EquityJusticeAnalyzer
from analyzer.climate_analysis import ClimateAnalyzer
from analyzer.risk_quantifier import RiskQuantifier
...
_PUBLIC    = os.path.join(os.path.dirname(_HERE), 'public')

app = Flask(__name__, template_folder=_TEMPLATES, static_folder=_PUBLIC)
...
def styles():
    return send_from_directory(_PUBLIC, 'styles.css')


@app.route('/script.js')
def script():
    return send_from_directory(_PUBLIC, 'script.js')
